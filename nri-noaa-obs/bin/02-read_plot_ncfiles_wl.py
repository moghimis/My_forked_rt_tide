...
filename = '../inp/ncom/ncom_glb_regp01_2012-3-4-5.nc'
ncf   = netCDF4.Dataset(filename)
ncvar = ncf.variables
time      = ncvar['time']
time_unit = time.units
utime       = netcdftime.utime(time.units)
date_ncom   = utime.num2date(time)
elev_ncom   = ncvar['surf_el'][:]
lat_ncom   = ncvar['lat'][:]
lon_ncom   = -ncvar['lon'][:]
lat_ncom , lon_ncom = np.meshgrid(lat_ncom,lon_ncom)

filename = '../inp/wl_8656483_Beaufort_NC/data.nc'
ncf=netCDF4.Dataset(filename)
ncvar=ncf.variables
time      = ncvar['time']
time_unit = time.units
utime       = netcdftime.utime(time.units)
date_beau   = utime.num2date(time)
elev_beau   = ncvar['elev'][:]
lat_beau    = ncvar['lat'][:].item()
lon_beau    = ncvar['lon'][:].item()
######
lat = lat_beau
lon = lon_beau
dist2=np.sqrt ((lat_ncom-lat)**2+(lon_ncom-lon)**2)
distm2=dist2.min()
idx2=np.where(dist2==distm2)    
[jnum],[inum]=idx2
elev_ncom_beau = elev_ncom[:,jnum,inum]
lat_ncom_beau  = lat_ncom [  jnum,inum]
lon_ncom_beau  = lon_ncom [  jnum,inum]
########################################################
filename = '../inp/wl_8658163_Wrightsville_Beach_NC/data.nc'
ncf=netCDF4.Dataset(filename)
ncvar=ncf.variables
time      = ncvar['time']
time_unit = time.units
utime       = netcdftime.utime(time.units)
date_wrig   = utime.num2date(time)
elev_wrig   = ncvar['elev'][:]
lat_wrig    = ncvar['lat'][:].item()
lon_wrig    = ncvar['lon'][:].item()
######
lat = lat_wrig
lon = lon_wrig
dist2=np.sqrt ((lat_ncom-lat)**2+(lon_ncom-lon)**2)
...
idx2=np.where(dist2==distm2)    
[jnum],[inum]=idx2
elev_ncom_wrig = elev_ncom[:,jnum,inum]
lat_ncom_wrig  = lat_ncom [  jnum,inum]
lon_ncom_wrig  = lon_ncom [  jnum,inum]
#########################################################
lats  = [lat_ncom_beau  , lat_beau   ,lat_wrig  , lat_ncom_wrig ]
lons  = [lon_ncom_beau  , lon_beau   ,lon_wrig  , lon_ncom_wrig ]
elevs = [elev_ncom_beau , elev_beau  ,elev_wrig , elev_ncom_wrig ]
dates = [date_ncom      , date_beau  ,date_wrig , date_ncom ]
label = ['NCOM_Beaufort'    , 'Beaufort' , 'Wrightsville','NCOM_Wrightsville']

plt.figure()
for iplot in range (len(lons)):
    plt.plot_date(dates[iplot],elevs[iplot]-elevs[iplot].mean(),color=ps.cl[iplot],
                  label=label[iplot],linestyle='-',marker='None')
...
y.dates=dates_obs
y.elevation=elevation_obs

package = y.astronomic(y.dates)
(y.zeta, y.nu, y.nup, y.nupp, y.kap_p, y.ii, y.R, y.Q, y.T, y.jd, y.s, y.h, y.N, y.p, y.p1) = package
ray = 1.0
(y.speed_dict, y.key_list) = y.which_constituents(len(y.dates),
                                                      package,
                                                      rayleigh_comp = ray)

y.constituents()

if y.missing_data == 'fill':
        y.dates_filled, y.elevation_filled = y.missing(y.missing_data, y.dates, y.elevation)
        y.write_file( y.dates_filled,
                        y.elevation_filled,
                        fname='outts_filled.dat')
...
if y.filter:
        for item in y.filter.split(','):
            if item in ['mstha', 'wavelet', 'cd', 'boxcar', 'usgs', 'doodson', 'lecolazet1', 'kalman', 'transform']:# 'lecolazet', 'godin', 'sfa']:
                filtered_dates, result = y.filters(item, y.dates, y.elevation)
                y.write_file(filtered_dates, result, fname='outts_filtered_%s.dat' % (item,))
                y_dates_filter= filtered_dates
                y_eleva_filter= result              
        (y.speed_dict, y.key_list) = y.which_constituents(len(y.dates),package,rayleigh_comp = ray)
...
ymean_elev= y.elevation.mean()



plot(  x.dates        ,   x.elevation      - xmean_elev     ,'k', label='elev ')         
plot(  y.dates        ,   y.elevation      - ymean_elev     ,'r', label='elev obs')         

labelx='Filter  '+ x.filter
labely='Filter obs   '+ y.filter
plot(x_dates_filter , x_eleva_filter- xmean_elev ,'b' , label=labelx)
plot(y_dates_filter , y_eleva_filter- ymean_elev ,'g', label=labely)
...
legend() 
savefig('out.png',dpi=600)



if outnc:
    units =time.units
    utime2=netcdftime.utime(units)
    yd_time=x.dates
    yd_sec=utime2.date2num(yd_time)
    elev_orig=x.elevation-x.elevation.mean()
    
    
    ft_sec=utime2.date2num(x_dates_filter)
    ft_lev_new =  interp( yd_sec,ft_sec, x_eleva_filter)
    elev_filter = ft_lev_new-x.elevation.mean()
    
    
    outfile=filename[:-3]+'_filtered.nc'
    outnc=netCDF4.Dataset(outfile,'w',format='NETCDF3_CLASSIC')
    outnc.history  = 'SM tidal analysis 2012'
    outnc.depth = x.elevation.mean()
    ...
    p1 = outnc.createVariable('filter','f8',('time',))
    p1.units = 'm'
    p1.missing_value = -99.0
    p1[:]=elev_filter
    ...
    p4 = outnc.createVariable('dep','f8')
    p4.units = 'm'
    p4.missing_value = -99.0
    p4[:]=x.elevation.mean()
...
# make obs also in nc file
if True:
    base_date=y_dates_filter[0]    #datetime.datetime(2012, 5, 1, 19, 0)
    units ="seconds since 2012-05-01 00:00:00"
    utime2=netcdftime.utime(units)
    yd_time=y.dates
    yd_sec=utime2.date2num(yd_time)
    elev_orig=y.elevation-y.elevation.mean()
   
    ft_sec=utime2.date2num(y_dates_filter)
    ft_lev_new =  interp( yd_sec,ft_sec, y_eleva_filter)
    elev_filter = ft_lev_new-y.elevation.mean()
    
    outfile=filename2[:-3]+'_filtered.nc'
    outnc=netCDF4.Dataset(outfile,'w',format='NETCDF3_CLASSIC')
    outnc.history  = 'SM tidal analysis 2012'
    outnc.depth = y.elevation.mean()
