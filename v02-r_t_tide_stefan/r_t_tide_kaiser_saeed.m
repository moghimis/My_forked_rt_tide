function [fu,fmaj,emaj,pha,epha,xout,stats]=r_t_tide_kaiser(t,xin,varargin)
%function [nameu,fu,fmaj,emaj,pha,epha,xout,stats]=r_t_tide_kaiser(t,xin,varargin)
...
% ADD KAISER WINDOW....
Hkai = zeros(length(h));
TCkai = zeros(length(TC));

wkai = kaiser(length(h),5);
...
while k<=length(args)
    if ischar(args{k}),
        ...
        switch lower(args{k}(1:3)),           
            case 'con',
                assert(iscell(args{k+1}),'Next arguement after "constit" needs to be a cell array');
                opt.constitnames = args{k+1};
            case 'err',
                assert(ischar(args{k+1}),'Next arguement after "error" needs to be a string');
                opt.errcalc=args{k+1};  
            case 'gre'
                assert(ischar(args{k+1}), 'Next arguement after "greenwichcorrflag" needs to be a string');
            ...
            case 'inf',
                ...
                dk = 5;
            case 'lat',
                assert(isnumeric(args{k+1}),'Arguement after "latitude" needs to be a number');
                opt.lat=args{k+1};
            case 'met',
                assert(ischar(args{k+1}),'Arguement after "method" needs to be a string');
                opt.method = args{k+1};      
            ...
            case 'nod'
                assert(ischar(args{k+1}), 'Arguement after "nodalcorrflag" needs to be a string');
                if strcmpi(args{k+1},'true'),   opt.nodalcorrflag = true;   end;
            case 'out',                
                assert(ischar(args{k+1}),'Arguement after "output" needs to be a string');
                switch args{k+1}
                    case 'none',
                        opt.fid=-1;
                    case 'screen',
            ...
            case 'pre',
                assert(isnumeric(args{k+1}) && isnumeric(args{k+2}),'Next two arguements after "pre" need to be numeric');
                dk = 3;
                opt.corr_fs=args{k+1};
                opt.corr_fac=args{k+2};
            case 'ray',
                assert(isnumeric(args{k+1}) || iscell(args{k+1}),'Arguement after "rayleight" needs to be numeric or a cell array');
                if isnumeric(args{k+1}),
                    opt.ray=args{k+1};
                else
                    opt.constitnames=args{k+1};
                    if iscellstr(opt.constitnames), opt.constitnames=char(opt.constitnames); end;
                end;
            case 'sav',
                assert(isa(args{k+1},'logical'),'Arguement after "savetestdata" needs to be true/false');
                opt.savetestdata = args{k+1};
            case 'sha',
                assert(iscell(args{k+1}),'Arguement after "shallow" needs to be a cell array');
                opt.shallownames=args{k+1};
            case 'spe',
                assert(ischar(args{k+1}),'Arguement after "specmethod" needs to be string');
                opt.specmethod = args{k+1};
            case 'sta',
                assert(isnumeric(args{k+1}),'Arguement after "starttime" needs to be numeric');
                opt.stime=args{k+1};
            ...
            case 'syn',
                assert(isnumeric(args{k+1}),'Arguement after "synth" needs to be numeric');
                opt.synth=args{k+1};
            case 'tco',
                assert(isnumeric(args{k+1}),'Arguement after "tconst" needs to be numeric');
                opt.tconst=args{k+1};
            case 'tse'
                assert(iscell(args{k+1}),'Arguement after "tseries" needs to be a cell array');
