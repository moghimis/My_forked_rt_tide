function [astro,ader] = r_t_astron(jd,varargin)
% r_t_astron Computes astronomical Variables
...
ader=[dtau;ader];
    % ####################################################################
    % Save test data
    if savetestdata && ~r_t_astron_saved
        r_t_astron_saved = true;
        outvals.astro = astro;
        outvals.ader = ader;
        writetestdata('r_t_astron',invals,outvals);
    end
    % ###################################################################
end