...
%        have been used, in r_t_tide I deal with the complex exponentials
...
% explanation of the plots, see 'help r_t_synth'):
%
% >r_t_synth('nrun',40,'error','.1*colrand(SY,-1)','time',[0:24*30],'tidecon',[1 0 0 60]);
%
% And this is for higher noise levels where the transformations is more
% nonlinear.
%
% >r_t_synth('nrun',40,'error','20*colrand(SY,-1)','time',[0:24*30],'tidecon',[1 0 0 60]);
% 
...
%
% >r_t_synth('nrun',40,'error','.01*(colrand(SY,-1)+i*colrand(SY,-1))','time',[0:24*30]);
%
% >r_t_synth('nrun',20,'error','10*(colrand(SY,-1)+i*colrand(SY,-1))','time',[0:24*30]);
%
...
%
% >r_t_synth('nrun',20,'error','.1*(1+i)*randn(SY)','time',[0:24*30],'boota','w');
%
