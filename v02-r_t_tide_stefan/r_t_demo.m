...
echo on
       ...
       [name,freq,tidecon,xout]=r_t_tide(tuk_time,tuk_elev,...
       ...
       echo off
       msl = mean(tuk_elev(~isnan(tuk_elev)));
       pout=r_t_predict(tuk_time,name,freq,tidecon,msl,...
                     'latitude',69+27/60,...
                     'synthesis',1,false,false);
...
subplot(412);
fsig=tidecon(:,1)>tidecon(:,2); % Significant peaks
semilogy([freq(~fsig),freq(~fsig)]',[.0005*ones(sum(~fsig),1),tidecon(~fsig,1)]','.-r');
line([freq(fsig),freq(fsig)]',[.0005*ones(sum(fsig),1),tidecon(fsig,1)]','marker','.','color','b');
line(freq,tidecon(:,2),'linestyle',':','color',[0 .5 0]);
set(gca,'ylim',[.0005 1],'xlim',[0 .5]);
xlabel('frequency (cph)');
text(freq,tidecon(:,1),name,'rotation',45,'vertical','base');
ylabel('Amplitude (m)');
...
subplot(413);
errorbar(freq(~fsig),tidecon(~fsig,3),tidecon(~fsig,4),'.r');
hold on;
errorbar(freq(fsig),tidecon(fsig,3),tidecon(fsig,4),'o');
hold off;
