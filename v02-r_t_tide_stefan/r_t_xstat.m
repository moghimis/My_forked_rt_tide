function [xstat,unit]=r_t_xstat(long,lat,varargin)
...
% #####################################################################
% Save test data
if savetestdata && ~r_t_xstat_saved
    r_t_xstat_saved = true;
   
    outvals.xstat = xstat;
    outvals.unit = unit;
  
    writetestdata('r_t_xstat',invals,outvals);
end
% #####################################################################
end
