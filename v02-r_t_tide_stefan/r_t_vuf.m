function [v,u,f]=r_t_vuf(t,ctime,ju, varargin)
% r_t_vuf Computes nodal modulation corrections.
% [V,U,F]=r_t_vuf(t,DATE,JU,LAT) returns the astronomical phase V, the 
...
% the CONST structure returned by r_t_getconsts) at a latitude LAT.
%
...
% Setup code for saving test data.
    savetestdata = false;
    lat = [];
    global r_t_vuf_saved 
    if nargin > 3
        varargs = parse_varargin(varargin);
        f = fieldnames(varargs);
        if ~isempty(intersect(f,'lat')), lat = varargs.lat;end;
        if ~isempty(intersect(f,'savetestdata')),    savetestdata = varargs.savetestdata; end;
        invals.t = t;
        invals.ctime = ctime;
        invals.ju = ju;
...
minres = 1/(t(2)-t(1));
[const,sat,shallow]=load_constits(t,ctime,minres,'savetestdata',savetestdata);

% Calculate astronomical arguments at mid-point of data time series.
[astro,ader]=r_t_astron(ctime,'savetestdata',savetestdata);
...
if ~isempty(lat) && isfinite(lat), % If we have a latitude, get nodal corrections.
...
% Code to save test data.
    if savetestdata && ~r_t_vuf_saved
        r_t_vuf_saved = true;
        %v,u,f
        outvals.v = v;
        outvals.u = u;
        outvals.f = f;
        writetestdata('r_t_vuf',invals,outvals);    
    end
% ##############################################    
end
