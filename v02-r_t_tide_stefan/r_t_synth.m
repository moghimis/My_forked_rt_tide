function [sm,lm,tcon]=r_t_synth(varargin)
...
%
% r_t_synth runs multiple realizations of this sequence. In each 
...
savetestdata = false;
global r_t_synth_saved
if nargin > 0
    varargs = parse_varargin(varargin);
    f = fieldnames(varargs);
    if ~isempty(intersect('freqs',f)),          freqs = varargs.freqs;  end;
    if ~isempty(intersect('tidecon',f)),        tcon = varargs.tidecon; end;
    if ~isempty(intersect('time',f)),           t = varargs.time;   end;
    if ~isempty(intersect('nrun',f)),           nrun = varargs.nrun;    end;
    if ~isempty(intersect('error',f)),          errorstr = varargs.error;   end;
    if ~isemtpy(intersect('boota',f)),          boota = varargs.boota;      end;
...
for k=1:nrun,
  ...
  [nameu,fu,tidecon,xout]=r_t_tide(y,'interval',t(2)-t(1),'output','none',...
                                 'rayleigh',freqs,'error',[boota 'boot']);
  [nameu2,fu2,tidecon2,xout2]=r_t_tide(y,'interval',t(2)-t(1),'output','none',...
...
if nargout==0,
 clear sm lm tcon
    ...
    % #########################################################################
    % Save test data
    if savetestdata && ~r_t_synth_saved
        r_t_synth_saved = true;
        outvals.y = y;   
        writetestdata('r_t_synth',invals,outvals);    
    end
    % ########################################################################
end
