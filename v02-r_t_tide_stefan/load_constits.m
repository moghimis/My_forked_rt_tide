% ---------------------------------------------------------------------
function [const,sat,shallow]=load_constits(t,ctime,minres,varargin)
% hpeg_t_getconsts Gets constituent data structures
% [CONST,SAT,SHALLOW]=hpeg_t_getconsts returns data structures holding
% information for tidal analyses.
%
% Variables are loaded from 'hpeg_t_constituents.mat', otherwise the
% ascii files 'tide3.dat' (provided with the IOS analysis package)
% and 'hpeg_t_equilib.dat' are read, and the results stored in
% 'hpeg_t_constituents.mat' for future use.
%
% [...]=hpeg_t_getconsts(TIME) recomputes the frequencies from the
...
% Version 1.0
    ...
    [const,sat,shallow] = read_constituentfile(ctime,minres);

    % ################################################################
    % Code for saving test data
    if savetestdata && ~load_constits_saved
        load_constits_saved = true;
        outvals.const = const;
        outvals.sat = sat;
        outvals.shallow = shallow;
        writetestdata('load_constits',invals,outvals);
    end
end
%-------------------------------------------------------------------------

function [const,sat,shallow] = read_constituentfile(ctime,~)
if exist('constituents.mat','file');
    load constituents
    %junk = 0;
else
    ...
    const=struct('name',char(zeros(nc,4)),... % constituent names
        'freq',empvec,...              % and frequencies (cph)
        'kmpr',char(zeros(nc,4)),... % names of comparisons
