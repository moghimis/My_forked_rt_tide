function pred=r_t_xtide(varargin)
% r_t_xtide Tidal prediction
% YOUT=r_t_xtide(STATION) makes a tidal prediction
...
% nearest to a given LONG,LAT, try r_t_xtide(LONG,LAT).
%
% The times of predicted tides are given by the next numerical argument
% (if any), e.g. [...]=r_t_xtide(STATION,TIM). 
...
% to the name, e.g.  r_t_xtide('tofino (2)',...).
%
...
if ~exist('r_t_xtide.mat','file'), % Read the harmonics file and make a mat file

  filnam='/usr/share/xtide/harmonics.txt';
  
  fprintf('\n********Can''t find mat-file r_t_xtide.mat ********\n\n');
  ...
  fprintf('Saving harmonic information to r_t_xtide.mat\n');
  save r_t_xtide xtide xharm
...
if nargin>0,
  ...
  if ischar(varargin{1}),  % Station  name given
